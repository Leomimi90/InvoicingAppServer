...
const signIn = async (req, res) => {
  try {
    ...
    await SignedInUser.create(signIn)
    let token = await signAccessToken(signIn.user_id)


    if (signedIn.length == 0 || signedIn[signedIn.length - 1].user_agent != req.headers[user_agent]) {
...
const activateUserAccount = async (req, res) => {
  try {
    ...
    await SignedUpUser.findByIdAndUpdate(user._id, { $set: { active: true } })
